 ...
 */
const ES_LABEL_SE   = 0x00000000;   // Label in the south-east - the default for LCARS components
const ES_LABEL_S    = 0x00000010;   // Label in the south
const ES_LABEL_SW   = 0x00000020;   // Label in the south-west
const ES_LABEL_W    = 0x00000030;   // Label in the west
const ES_LABEL_NW   = 0x00000040;   // Label in the north-west
const ES_LABEL_N    = 0x00000050;   // Label in the north
const ES_LABEL_NE   = 0x00000060;   // Label in the north-east
const ES_LABEL_E    = 0x00000070;   // Label in the east
const ES_LABEL_C    = 0x00000080;   // Label in the center
...
LCARSComponent = function(name, label, x, y, properties) {
    this.name = name;
    this.label = label;
    this.x = x;
    this.y = y;
    ...
    this.color = LCARSComponent.getColor(properties);
    this.overColor = this.getOverColor(properties);
    this.downColor = this.getDownColor(properties);
    this.textColor = this.getTextColor(properties);
...
LCARSComponent.prototype.getShapeElement = function() {
    return document.getElementById(this.name + "_shape");
}

LCARSComponent.prototype.getTextElement = function() {
    return document.getElementById(this.name + "_text");
}

LCARSComponent.prototype.addEventListener = function(event, callback) {
    this.getElement().addEventListener(event, callback, false);
}

LCARSComponent.getColor = function(properties) {
    return LCARS.getColor(properties);
}
...
LCARSComponent.prototype.getComponentDynamics = function() {
    var htmlString = "";
    
    if(this.static != ES_STATIC) {
        htmlString += " onmouseover=" + '"' + "evt.target.setAttribute('fill','" + this.overColor + "')" + '"';
        htmlString += " onmousedown=" + '"' + "evt.target.setAttribute('fill','" + this.downColor + "')" + '"';
        htmlString += " onmouseup=" + '"' + "evt.target.setAttribute('fill','" + this.color + "')" + '"';
        htmlString += " onmouseout=" + '"' + "evt.target.setAttribute('fill','" + this.color + "')" + '"';
    }
...
LCARSComponent.prototype.onmouseover = function() {
}

LCARSComponent.prototype.onmouseout = function() {
}

LCARSComponent.prototype.onmousedown = function() {
}

LCARSComponent.prototype.onmouseup = function() {
}
...
LCARSComponent.prototype.getHTML = function getHTML() {
    var htmlString = "<g id='" + this.name + "' transform='translate(" + this.x + "," + this.y + ")'>";
    
    htmlString += this.drawShape();
    htmlString += " fill='" + this.color + "'";
    htmlString += this.getComponentDynamics() + "/>";
    htmlString += this.drawText();
    htmlString += "</g> ";
...
LCARSComponent.prototype.drawShape = function() {
    var rectString = " <path id='" + this.name + "_shape' d='M0,0";
...
LCARSComponent.prototype.drawText = function() {
    ...
    if(this.label != "" && this.label != null) {
        textString = "<text id='" + this.name + "_text' ";
        
        textString += "x='" + this.getTextX() + "' y='" + this.getTextY() +
        "' style='fill: " + this.textColor + "; font-family: " + LCARS_FONT + "; font-size: " + this.fontSize + "; -moz-user-select: none; -webkit-user-select: none; " +
        this.getTextAnchor() + "'>" +
        this.label + "</text>";
    }
...
LCARSComponent.prototype.setText = function(textString) {
    this.getTextElement().innerHTML = textString;
}

LCARSComponent.setShapeStyle = function(event, attribute, value) {
}
...
LCARSCorner.prototype.drawShape = function() {
    var pathString = " <path id='" + this.name + "_shape' d='M0,0";
...
LCARSCorner.prototype.drawText = function() {
    var textString = "<text id='" + this.name + "_text' ";
    var x;
    var y;
    var textAnchor = "text-anchor: ";
...
LCARSButton.prototype.clicked = function(event) {
}
...
function LCARSText(name, label, x, y, properties) {
    ...
    this.textColor = LCARSComponent.getColor(properties);
}
